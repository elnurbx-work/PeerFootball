...
export default async function NewInternalMatchPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <section className="mx-auto grid max-w-2xl gap-5 px-4 py-8"><Button asChild variant="ghost" className="w-fit"><Link href={`/clubs/${club.slug}/matches`}>← {t("matches.pages.new.back")}</Link></Button><Card><CardHeader><CardTitle>{t("matches.pages.new.internalTitle")}</CardTitle></CardHeader><CardContent><CreateInternalMatchForm clubId={club.id} /></CardContent></Card></section>;
}
