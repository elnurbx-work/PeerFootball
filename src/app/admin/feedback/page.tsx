      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid gap-4">
          {feedbacks.length ? feedbacks.map((feedback) => (
            <Card key={feedback.id} className="border-border bg-card text-foreground">
              <CardHeader>
                <CardTitle className="text-base">{feedback.user.name ?? feedback.user.username ?? feedback.user.email}</CardTitle>
                <p className="text-xs text-muted-foreground">{feedback.createdAt.toLocaleString("az-AZ")} · {feedback.status}</p>
              </CardHeader>
              <CardContent className="grid gap-4">
                ...
                <form action={updateFeedbackAction} className="grid gap-3 sm:grid-cols-[1fr_auto_auto_auto]">
                  ...
                  <Input name="adminNote" defaultValue={feedback.adminNote ?? ""} placeholder="Admin qeydi" className="border-border-strong bg-background" />
          ...
          )) : <p className="text-muted-foreground">Feedback yoxdur.</p>}
        </div>
