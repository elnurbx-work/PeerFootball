    ...
    <section id="pitches" className="landing-section bg-card">
      <div className="landing-container grid items-center gap-12 lg:grid-cols-[.78fr_1.22fr]">
        <div>
          ...
          <h2 className="mt-4 text-[clamp(2.25rem,5vw,4.25rem)] font-black leading-none tracking-[-0.055em] text-foreground">{copy.map.title}</h2>
          <p className="mt-5 text-lg leading-8 text-muted-foreground">{copy.map.body}</p>
        ...
        <div>
          ...
          <div className="map-canvas relative min-h-[420px] overflow-hidden rounded-[1.75rem] border border-border bg-map shadow-[0_24px_80px_rgba(15,23,42,.12)] sm:min-h-[520px]">
            ...
            <div className="absolute bottom-4 left-4 right-4 z-30 grid grid-cols-2 gap-2 rounded-2xl border border-white/70 bg-card/90 p-3 text-[10px] font-semibold text-muted-foreground shadow-lg backdrop-blur sm:grid-cols-4">
              {copy.map.legend.map((label, index) => <span key={label} className="flex items-center gap-2"><span className={cn("h-2.5 w-2.5 rounded-full", index === 0 ? "bg-accent" : index === 1 ? "bg-warning" : index === 2 ? "bg-primary" : "bg-muted-foreground")} />{label}</span>)}
            </div>
          </div>
        </div>
      </div>
...
function SelectedPitchCard({ copy }: { copy: LandingCopy }) {
  return (
    <article className="mt-8 rounded-[1.5rem] border border-border bg-surface-muted p-5">
      <div className="flex items-start gap-4">
        ...
        <div><h3 className="font-bold text-foreground">Arena Football Center</h3><p className="mt-1 text-sm text-muted-foreground">{copy.map.distance}</p></div>
      </div>
      <div className="mt-5 grid grid-cols-2 gap-3 text-sm text-muted-foreground">
        <span className="flex items-center gap-2"><CircleDot className="h-4 w-4 text-primary" />{copy.map.outdoor}</span>
        <span className="flex items-center gap-2"><CircleDot className="h-4 w-4 text-primary" />{copy.map.surface}</span>
        <span className="flex items-center gap-2"><CircleDot className="h-4 w-4 text-primary" />{copy.map.format}</span>
        <span className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-primary" />{copy.map.next}</span>
      </div>
