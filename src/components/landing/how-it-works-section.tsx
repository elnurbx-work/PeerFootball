...
export function HowItWorksSection({ copy }: { copy: LandingCopy }) {
  return (
    <section className="landing-section bg-[#0a1711] text-white">
      <div className="landing-container">
        <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-lime-400">{copy.steps.eyebrow}</p>
        ...
        <ol className="mt-14 grid gap-0 md:grid-cols-3">
          {copy.steps.items.map((step, index) => (
            <li key={step.title} className="group relative border-l border-white/15 pb-10 pl-10 last:pb-0 md:border-l-0 md:border-t md:pb-0 md:pl-0 md:pr-10 md:pt-10">
              <span className="absolute -left-[17px] top-0 grid h-8 w-8 place-items-center rounded-full border border-lime-400 bg-[#0a1711] text-xs font-black text-lime-400 md:-top-[17px] md:left-0">{index + 1}</span>
              <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white/5 text-lime-400"><Check aria-hidden="true" /></span>
