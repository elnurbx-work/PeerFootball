  ...
  return (
    <section id="matches" className="landing-section bg-background">
      <div className="landing-container">
        ...
        <div className="mt-12 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,.75fr)]">
          <article className="min-w-0 max-w-full overflow-hidden rounded-[1.75rem] bg-primary text-primary-foreground shadow-xl shadow-foreground/10">
            <div className="border-b border-primary-foreground/15 p-6 sm:p-8">
              <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
                ...
                <span className="flex min-w-0 items-center gap-1.5 text-sm text-primary-foreground/70"><MapPin className="h-4 w-4 shrink-0" />Arena Football Center</span>
              </div>
              <div className="mt-10 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 text-center sm:gap-4">
            ...
            <div className="grid gap-5 p-6 sm:grid-cols-[1fr_auto] sm:items-end sm:p-8">
              <div>
                <div className="flex min-w-0 flex-wrap items-center justify-between gap-2 text-sm"><span className="flex min-w-0 items-center gap-2"><Users className="h-4 w-4 shrink-0" />{copy.matches.joined}</span><span className="shrink-0">79%</span></div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">{copy.matches.positions.map((position) => <span key={position} className="rounded-lg border border-primary-foreground/20 px-2.5 py-1.5 text-xs">{position}</span>)}</div>
              </div>
          ...
          <div className="flex min-w-0 max-w-full flex-col gap-3">
            {nearbyMatches.map((match, index) => (
              <article key={match.teams} className="flex min-w-0 flex-1 items-center gap-3 rounded-[1.35rem] border border-border bg-card p-3 sm:gap-4 sm:p-4">
                ...
                <span className="shrink-0 rounded-full bg-primary/10 px-2.5 py-1 text-[10px] font-bold text-primary">{match.spots}</span>
              </article>
            ))}
