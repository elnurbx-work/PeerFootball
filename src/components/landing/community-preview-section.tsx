...
function Actions({ copy, likes, comments }: { copy: LandingCopy; likes: number; comments: number }) {
  return <div className="flex items-center gap-5 border-t border-border pt-3 text-xs font-semibold text-muted-foreground"><span className="flex items-center gap-1.5"><Heart className="h-4 w-4" />{likes} {copy.community.likes}</span><span className="flex items-center gap-1.5"><MessageCircle className="h-4 w-4" />{comments} {copy.community.comments}</span><span className="ml-auto flex items-center gap-1.5"><Repeat2 className="h-4 w-4" />{copy.community.repost}</span></div>;
}
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
          <article className="overflow-hidden rounded-[1.75rem] border border-border bg-card">
            ...
            <div className="relative mx-4 aspect-[16/9] overflow-hidden rounded-2xl bg-brand sm:mx-6">
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-6 pt-20 text-white"><p className="text-xs font-bold uppercase tracking-wider text-accent">Full time</p><p className="mt-1 text-3xl font-black">FALCONS 4—2 NORTHSIDE</p></div>
            </div>
