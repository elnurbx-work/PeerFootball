...
export function MatchTabs({ match, options, selectedVideoId, onSelectVideo, seekRequest, onTimestampClick, seekMessage }: {
  ...
  useEffect(() => { if (seekRequest) setActive("videos"); }, [seekRequest]);
  ...
  const editable = match.permissions.canManagePlayers;
  return <Card className="min-w-0 overflow-hidden">
    <div className="grid grid-cols-3 border-b px-1 sm:px-6">
      {tabs.map((tab) => <button key={tab} type="button" onClick={() => setActive(tab)} className={`min-w-0 border-b-2 px-1 py-3 text-xs font-medium transition sm:px-5 sm:py-4 sm:text-sm ${active === tab ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>{tab === "videos" ? t("matches.tabs.videos") : tab === "players" ? t("matches.tabs.players") : t("matches.tabs.comments")}</button>)}
    </div>
