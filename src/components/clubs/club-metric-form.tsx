  ...
  return (
    <form className="grid gap-3" onSubmit={handleSubmit}>
      <div className="grid gap-3 md:grid-cols-[1fr_120px]">
        <Input name="name" placeholder="Metric name" defaultValue={metric?.name ?? ""} required maxLength={50} />
        <Input name="order" type="number" min={1} max={6} defaultValue={metric?.order ?? ""} />
      </div>
      <Textarea name="description" placeholder="Description" defaultValue={metric?.description ?? ""} rows={3} />
      {message ? <p className="text-sm text-muted-foreground">{message}</p> : null}
      <Button type="submit" disabled={pending} className="w-fit">
        <Save className="h-4 w-4" />
        {pending ? "Saving..." : metric ? "Save metric" : "Create metric"}
      </Button>
    </form>
  );
}
