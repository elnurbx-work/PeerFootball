...
function createDirectNotification(): AppNotification {
  return {
    id: "notification-1",
    type: "MESSAGE",
    title: null,
...
test("urlBase64ToUint8Array converts base64url", () => {
...
test("direct message push payload preserves privacy and URL", () => {
  const directPayload = buildPushPayloadFromNotification(createDirectNotification(), "az");
...
test("match notification payload", () => {
...
test("normalizePushPayload fallbacks", () => {
...
test("shouldDeletePushSubscription handles status codes", () => {
